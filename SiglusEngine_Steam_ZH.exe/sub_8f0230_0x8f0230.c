// 函数: sub_8f0230
// 地址: 0x8f0230
// 来自: E:\Download\Various Files\CLANNAD HD Edition\CLANNAD\SiglusEngine_Steam.exe

void var_148
__builtin_memcpy(&var_148, arg1, 0x144)
char* var_54
char* edi = var_54
int32_t var_40
int32_t edx = var_40
int32_t var_28
int32_t i_1 = var_28 + 1
int32_t i

do
    i = i_1
    int32_t temp0_1 = edx
    int32_t var_48
    edx += var_48
    
    if (temp0_1 + var_48 s>= 0)
        int32_t var_44
        int32_t temp3_1
        
        do
            int32_t var_a0
            int32_t var_50
            
            if (edi s>= var_50)
                int32_t var_4c
                
                if (edi s> var_4c)
                    return 
                
                char* var_3c
                int32_t ebx_1
                ebx_1.b = var_3c[3]
                
                if (ebx_1 != 0)
                    void* var_98
                    int32_t eax_1
                    eax_1.b = *(var_98 + var_a0)
                    int32_t var_b4
                    
                    if (var_b4 != 0)
                        eax_1.b = not.b(eax_1.b)
                    
                    char var_b0
                    int32_t var_ac
                    int32_t eax_3 = (eax_1 << var_b0) + var_ac
                    
                    if (eax_3 s< 0)
                        eax_3 = 0
                    else if (eax_3 s> 0xff)
                        eax_3 = 0xff
                    
                    if (eax_3 != 0)
                        int32_t ebx_12 = *(*((eax_3 << 2) + &data_4e3da80) + (ebx_1 << 2))
                        
                        if (ebx_12 != 0)
                            int32_t edx_2 = *((ebx_12 << 2) + &data_4e3da80)
                            int32_t ebx_16
                            ebx_16.b = var_3c[2]
                            int32_t var_88
                            int32_t ebx_17 = ebx_16 + var_88
                            int32_t ebx_25
                            
                            if (ebx_16 + var_88 s< 0)
                                if (ebx_17 s< 0xffffff01)
                                    ebx_17 = 0xffffff01
                                
                                int32_t ebx_28
                                ebx_28.b = edi[2]
                                ebx_25 = *((ebx_28 << 2) + *((ebx_17 << 2) + data_4ced578))
                            else
                                if (ebx_17 s> 0xff)
                                    ebx_17 = 0xff
                                
                                int32_t ebx_20
                                ebx_20.b = edi[2]
                                ebx_25 = *(((ebx_20 - 0xff) << 2) + *((ebx_17 << 2) + data_4ced578))
                                    + 0xff
                            
                            int32_t eax_6
                            eax_6.b = edi[2]
                            edi[2] = eax_6.b + (*(((ebx_25 - eax_6) << 2) + edx_2)).b
                            int32_t ebx_34
                            ebx_34.b = var_3c[1]
                            int32_t var_84
                            int32_t ebx_35 = ebx_34 + var_84
                            int32_t ebx_43
                            
                            if (ebx_34 + var_84 s< 0)
                                if (ebx_35 s< 0xffffff01)
                                    ebx_35 = 0xffffff01
                                
                                int32_t ebx_46
                                ebx_46.b = edi[1]
                                ebx_43 = *((ebx_46 << 2) + *((ebx_35 << 2) + data_4ced578))
                            else
                                if (ebx_35 s> 0xff)
                                    ebx_35 = 0xff
                                
                                int32_t ebx_38
                                ebx_38.b = edi[1]
                                ebx_43 = *(((ebx_38 - 0xff) << 2) + *((ebx_35 << 2) + data_4ced578))
                                    + 0xff
                            
                            int32_t eax_10
                            eax_10.b = edi[1]
                            edi[1] = eax_10.b + (*(((ebx_43 - eax_10) << 2) + edx_2)).b
                            int32_t ebx_52
                            ebx_52.b = *var_3c
                            int32_t var_80
                            int32_t ebx_53 = ebx_52 + var_80
                            int32_t ebx_61
                            
                            if (ebx_52 + var_80 s< 0)
                                if (ebx_53 s< 0xffffff01)
                                    ebx_53 = 0xffffff01
                                
                                int32_t ebx_64
                                ebx_64.b = *edi
                                ebx_61 = *((ebx_64 << 2) + *((ebx_53 << 2) + data_4ced578))
                            else
                                if (ebx_53 s> 0xff)
                                    ebx_53 = 0xff
                                
                                int32_t ebx_56
                                ebx_56.b = *edi
                                ebx_61 = *(((ebx_56 - 0xff) << 2) + *((ebx_53 << 2) + data_4ced578))
                                    + 0xff
                            
                            int32_t eax_14
                            eax_14.b = *edi
                            *edi = eax_14.b + (*(((ebx_61 - eax_14) << 2) + edx_2)).b
            
            int32_t eax = var_a0 + 1
            int32_t var_bc
            
            if (eax s>= var_bc)
                eax = 0
            
            var_a0 = eax
            edi = &edi[4]
            temp3_1 = edx
            edx -= var_44
        while (temp3_1 - var_44 s>= 0)
    
    i_1 = i - 1
while (i != 1)
