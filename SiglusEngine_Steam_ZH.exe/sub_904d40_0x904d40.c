// 函数: sub_904d40
// 地址: 0x904d40
// 来自: E:\Download\Various Files\CLANNAD HD Edition\CLANNAD\SiglusEngine_Steam.exe

void var_148
__builtin_memcpy(&var_148, arg1, 0x144)
char* var_54
char* edi = var_54
int32_t var_40
int32_t edx = var_40
int32_t var_28
int32_t i_1 = var_28 + 1
int32_t i

do
    i = i_1
    int32_t temp3_1 = edx
    int32_t var_48
    edx += var_48
    
    if (temp3_1 + var_48 s>= 0)
        int32_t var_44
        int32_t temp6_1
        
        do
            int32_t var_50
            
            if (edi s>= var_50)
                int32_t var_4c
                
                if (edi s> var_4c)
                    return 
                
                char* var_3c
                int32_t ebx_1
                ebx_1.b = var_3c[3]
                
                if (ebx_1 != 0)
                    int32_t var_11c
                    int32_t ebx_7 = *(*((ebx_1 << 2) + &data_4e3da80) + var_11c)
                    
                    if (ebx_7 != 0)
                        int32_t ecx_3 = ebx_7 << 2
                        int32_t eax_1
                        eax_1.b = edi[3]
                        int32_t eax = eax_1 << 2
                        int32_t ecx_7 = *(*(ecx_3 + &data_4dae580) + eax)
                        
                        if (ecx_7 != 0)
                            int32_t edx_6 = *(*(ecx_3 + &data_4dee980) + eax)
                            int32_t ebx_9
                            ebx_9.b = var_3c[2]
                            int32_t eax_2
                            eax_2.b = not.b(ebx_9.b)
                            int32_t var_f8
                            int32_t var_ec
                            int32_t var_e0
                            int32_t var_d4
                            int32_t var_d0
                            int32_t eax_7 = var_f8 + *(((var_e0 + *(((eax_2
                                + *(((ebx_9 - eax_2) << 2) + var_d0) - var_e0) << 2) + var_d4)
                                - var_f8) << 2) + var_ec)
                            int32_t ebx_22
                            ebx_22.b = var_3c[1]
                            int32_t eax_8
                            eax_8.b = not.b(ebx_22.b)
                            int32_t var_f4
                            int32_t var_e8
                            int32_t var_dc
                            int32_t eax_13 = var_f4 + *(((var_dc + *(((eax_8
                                + *(((ebx_22 - eax_8) << 2) + var_d0) - var_dc) << 2) + var_d4)
                                - var_f4) << 2) + var_e8)
                            int32_t ebx_35
                            ebx_35.b = *var_3c
                            int32_t eax_14
                            eax_14.b = not.b(ebx_35.b)
                            int32_t var_f0
                            int32_t var_e4
                            int32_t var_d8
                            int32_t eax_19 = var_f0 + *(((var_d8 + *(((eax_14
                                + *(((ebx_35 - eax_14) << 2) + var_d0) - var_d8) << 2) + var_d4)
                                - var_f0) << 2) + var_e4)
                            
                            if (ebx_7 != 0xff)
                                int32_t ebx_53
                                ebx_53.b = edi[3]
                                
                                if (ebx_53 != 0)
                                    int32_t eax_22 = *((ebx_7 << 2) + &data_4d6e180)
                                    *((eax_13 << 2) + eax_22)
                                    *((eax_19 << 2) + eax_22)
                                    int32_t eax_23
                                    eax_23.b = edi[2]
                                    edi[2] = (divu.dp.d(mulu.dp.d(eax_23, edx_6), ecx_7)).b
                                    int32_t eax_28
                                    eax_28.b = edi[1]
                                    edi[1] = (divu.dp.d(mulu.dp.d(eax_28, edx_6), ecx_7)).b
                                    int32_t eax_33
                                    eax_33.b = *edi
                                    *edi = divu.dp.d(mulu.dp.d(eax_33, edx_6), ecx_7).b
                                    edi[3] = ecx_7.b
                                else
                                    edi[2] = eax_7.b
                                    edi[1] = eax_13.b
                                    *edi = eax_19.b
                                    edi[3] = ebx_7.b
                            else
                                edi[2] = eax_7.b
                                edi[1] = eax_13.b
                                *edi = eax_19.b
                                edi[3] = ebx_7.b
            
            edi = &edi[4]
            temp6_1 = edx
            edx -= var_44
        while (temp6_1 - var_44 s>= 0)
    
    i_1 = i - 1
while (i != 1)
