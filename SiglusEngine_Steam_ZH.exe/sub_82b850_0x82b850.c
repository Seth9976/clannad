// 函数: sub_82b850
// 地址: 0x82b850
// 来自: E:\Download\Various Files\CLANNAD HD Edition\CLANNAD\SiglusEngine_Steam.exe

void var_148
__builtin_memcpy(&var_148, arg1, 0x144)
void* var_134
void* edi = var_134
int32_t i_2
int32_t i_1 = i_2
int32_t i

do
    int32_t j_2
    int32_t j_1 = j_2
    int32_t j
    
    do
        char* var_124
        int32_t ebx_1
        ebx_1.b = var_124[3]
        
        if (ebx_1 != 0)
            int32_t edx_1 = *((ebx_1 << 2) + &data_4e3da80)
            int32_t ebx_5
            ebx_5.b = var_124[2]
            int32_t eax_1
            eax_1.b = not.b(ebx_5.b)
            int32_t eax_7
            eax_7.b = *(edi + 2)
            int32_t var_f8
            int32_t var_ec
            int32_t var_e0
            int32_t var_d4
            int32_t var_d0
            int32_t* ebx_19 = ((var_f8 + *(((var_e0
                + *(((eax_1 + *(((ebx_5 - eax_1) << 2) + var_d0) - var_e0) << 2) + var_d4)
                - var_f8) << 2) + var_ec)) << 2) + *((eax_7 << 2) + &data_4e3da80)
            int32_t eax_11
            eax_11.b = *(edi + 2)
            *(edi + 2) = eax_11.b + (*(((*ebx_19 - eax_11) << 2) + edx_1)).b
            int32_t ebx_24
            ebx_24.b = var_124[1]
            int32_t eax_13
            eax_13.b = not.b(ebx_24.b)
            int32_t eax_19
            eax_19.b = *(edi + 1)
            int32_t var_f4
            int32_t var_e8
            int32_t var_dc
            int32_t* ebx_38 = ((var_f4 + *(((var_dc
                + *(((eax_13 + *(((ebx_24 - eax_13) << 2) + var_d0) - var_dc) << 2) + var_d4)
                - var_f4) << 2) + var_e8)) << 2) + *((eax_19 << 2) + &data_4e3da80)
            int32_t eax_23
            eax_23.b = *(edi + 1)
            *(edi + 1) = eax_23.b + (*(((*ebx_38 - eax_23) << 2) + edx_1)).b
            int32_t ebx_43
            ebx_43.b = *var_124
            int32_t eax_25
            eax_25.b = not.b(ebx_43.b)
            int32_t eax_31
            eax_31.b = *edi
            int32_t var_f0
            int32_t var_e4
            int32_t var_d8
            int32_t* ebx_57 = ((var_f0 + *(((var_d8
                + *(((eax_25 + *(((ebx_43 - eax_25) << 2) + var_d0) - var_d8) << 2) + var_d4)
                - var_f0) << 2) + var_e4)) << 2) + *((eax_31 << 2) + &data_4e3da80)
            int32_t eax_35
            eax_35.b = *edi
            *edi = eax_35.b + (*(((*ebx_57 - eax_35) << 2) + edx_1)).b
        
        edi += 4
        j = j_1
        j_1 -= 1
    while (j != 1)
    int32_t var_130
    edi += var_130
    i = i_1
    i_1 -= 1
while (i != 1)
