// 函数: sub_883480
// 地址: 0x883480
// 来自: E:\Download\Various Files\CLANNAD HD Edition\CLANNAD\SiglusEngine_Steam.exe

void var_148
__builtin_memcpy(&var_148, arg1, 0x144)
char* var_3c
char* esi_1 = var_3c
char* var_54
char* edi = var_54
int32_t var_40
int32_t edx = var_40
int32_t result_1
int32_t result = result_1
int32_t var_34
int32_t ebx = var_34
int32_t var_28
int32_t i_1 = var_28 + 1
int32_t i

do
    int32_t temp0_1 = edx
    int32_t var_48
    edx += var_48
    
    if (temp0_1 + var_48 s>= 0)
        int32_t var_44
        int32_t temp2_1
        
        do
            int32_t var_50
            
            if (edi s>= var_50)
                int32_t var_4c
                
                if (edi s> var_4c)
                    return result
                
                int32_t ebx_2
                ebx_2.b = esi_1[3]
                
                if (ebx_2 != 0)
                    int32_t edx_2 = *((ebx_2 << 2) + &data_4e3da80)
                    int32_t ebx_6
                    ebx_6.b = esi_1[2]
                    int32_t var_88
                    int32_t ebx_7 = ebx_6 + var_88
                    int32_t ebx_15
                    
                    if (ebx_6 + var_88 s< 0)
                        if (ebx_7 s< 0xffffff01)
                            ebx_7 = 0xffffff01
                        
                        int32_t ebx_18
                        ebx_18.b = edi[2]
                        ebx_15 = *((ebx_18 << 2) + *((ebx_7 << 2) + data_4ced578))
                    else
                        if (ebx_7 s> 0xff)
                            ebx_7 = 0xff
                        
                        int32_t ebx_10
                        ebx_10.b = edi[2]
                        ebx_15 = *(((ebx_10 - 0xff) << 2) + *((ebx_7 << 2) + data_4ced578)) + 0xff
                    
                    int32_t eax_2
                    eax_2.b = edi[2]
                    edi[2] = eax_2.b + (*(((ebx_15 - eax_2) << 2) + edx_2)).b
                    int32_t ebx_24
                    ebx_24.b = esi_1[1]
                    int32_t var_84
                    int32_t ebx_25 = ebx_24 + var_84
                    int32_t ebx_33
                    
                    if (ebx_24 + var_84 s< 0)
                        if (ebx_25 s< 0xffffff01)
                            ebx_25 = 0xffffff01
                        
                        int32_t ebx_36
                        ebx_36.b = edi[1]
                        ebx_33 = *((ebx_36 << 2) + *((ebx_25 << 2) + data_4ced578))
                    else
                        if (ebx_25 s> 0xff)
                            ebx_25 = 0xff
                        
                        int32_t ebx_28
                        ebx_28.b = edi[1]
                        ebx_33 = *(((ebx_28 - 0xff) << 2) + *((ebx_25 << 2) + data_4ced578)) + 0xff
                    
                    int32_t eax_6
                    eax_6.b = edi[1]
                    edi[1] = eax_6.b + (*(((ebx_33 - eax_6) << 2) + edx_2)).b
                    int32_t ebx_42
                    ebx_42.b = *esi_1
                    int32_t var_80
                    int32_t ebx_43 = ebx_42 + var_80
                    int32_t ebx_51
                    
                    if (ebx_42 + var_80 s< 0)
                        if (ebx_43 s< 0xffffff01)
                            ebx_43 = 0xffffff01
                        
                        int32_t ebx_54
                        ebx_54.b = *edi
                        ebx_51 = *((ebx_54 << 2) + *((ebx_43 << 2) + data_4ced578))
                    else
                        if (ebx_43 s> 0xff)
                            ebx_43 = 0xff
                        
                        int32_t ebx_46
                        ebx_46.b = *edi
                        ebx_51 = *(((ebx_46 - 0xff) << 2) + *((ebx_43 << 2) + data_4ced578)) + 0xff
                    
                    int32_t eax_10
                    eax_10.b = *edi
                    *edi = eax_10.b + (*(((ebx_51 - eax_10) << 2) + edx_2)).b
            
            edi = &edi[4]
            temp2_1 = edx
            edx -= var_44
        while (temp2_1 - var_44 s>= 0)
    
    result &= 1
    
    if (result != 0)
        int32_t var_24
        esi_1 = &esi_1[var_24]
        int32_t temp4_1 = ebx
        int32_t var_30
        ebx += var_30
        
        if (temp4_1 + var_30 s>= 0)
            int32_t var_20
            esi_1 = &esi_1[var_20]
            int32_t var_2c
            ebx -= var_2c
    else
        esi_1 = &esi_1[ebx]
    
    i = i_1
    i_1 -= 1
while (i != 1)
return result
