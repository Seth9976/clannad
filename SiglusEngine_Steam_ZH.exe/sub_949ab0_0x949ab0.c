// 函数: sub_949ab0
// 地址: 0x949ab0
// 来自: E:\Download\Various Files\CLANNAD HD Edition\CLANNAD\SiglusEngine_Steam.exe

void var_148
__builtin_memcpy(&var_148, arg1, 0x144)
int32_t* var_14
int32_t* esi_4 = var_14
char* var_1c
char* edi = var_1c
int32_t i_2
int32_t i_1 = i_2
char result
int32_t i

do
    int32_t eax_1 = *esi_4
    void* esi_1 = &esi_4[1]
    
    if (eax_1 != 0)
        int32_t var_c
        
        if (eax_1 s> 0)
            edi = &edi[var_c]
        else
            edi -= var_c
    
    int32_t eax_2 = *esi_1
    
    if (eax_2 != 0)
        int32_t var_8
        
        if (eax_2 s> 0)
            edi = &edi[var_8]
        else
            edi -= var_8
    
    char* var_18
    int32_t eax_3
    eax_3.b = var_18[2]
    int32_t eax_5
    eax_5.b = var_18[1]
    int32_t eax_7
    eax_7.b = *var_18
    uint32_t edx_5 = (*(&data_4e3d680 + (eax_3 << 2)) + *(&data_4e3e280 + (eax_5 << 2))
        + *(&data_4e3de80 + (eax_7 << 2))) u>> 8
    int32_t var_cc
    int32_t eax_9 = *(&data_4e3da80 + var_cc)
    int32_t ebx_5
    ebx_5.b = var_18[2]
    int32_t eax_11 = edx_5 + *(((ebx_5 - edx_5) << 2) + eax_9)
    int32_t eax_12
    eax_12.b = not.b(eax_11.b)
    int32_t var_d0
    int32_t eax_13 = eax_12 + *(((eax_11 - eax_12) << 2) + var_d0)
    int32_t var_88
    int32_t ebx_14 = eax_13 + var_88
    int32_t ebx_22
    
    if (eax_13 + var_88 s< 0)
        if (ebx_14 s< 0xffffff01)
            ebx_14 = 0xffffff01
        
        int32_t ebx_25
        ebx_25.b = edi[2]
        ebx_22 = *((ebx_25 << 2) + *((ebx_14 << 2) + data_4ced578))
    else
        if (ebx_14 s> 0xff)
            ebx_14 = 0xff
        
        int32_t ebx_17
        ebx_17.b = edi[2]
        ebx_22 = *(((ebx_17 - 0xff) << 2) + *((ebx_14 << 2) + data_4ced578)) + 0xff
    
    int32_t eax_16
    eax_16.b = edi[2]
    int32_t var_118
    edi[2] = eax_16.b + (*(((ebx_22 - eax_16) << 2) + var_118)).b
    int32_t ebx_31
    ebx_31.b = var_18[1]
    int32_t eax_19 = edx_5 + *(((ebx_31 - edx_5) << 2) + eax_9)
    int32_t eax_20
    eax_20.b = not.b(eax_19.b)
    int32_t eax_21 = eax_20 + *(((eax_19 - eax_20) << 2) + var_d0)
    int32_t var_84
    int32_t ebx_40 = eax_21 + var_84
    int32_t ebx_48
    
    if (eax_21 + var_84 s< 0)
        if (ebx_40 s< 0xffffff01)
            ebx_40 = 0xffffff01
        
        int32_t ebx_51
        ebx_51.b = edi[1]
        ebx_48 = *((ebx_51 << 2) + *((ebx_40 << 2) + data_4ced578))
    else
        if (ebx_40 s> 0xff)
            ebx_40 = 0xff
        
        int32_t ebx_43
        ebx_43.b = edi[1]
        ebx_48 = *(((ebx_43 - 0xff) << 2) + *((ebx_40 << 2) + data_4ced578)) + 0xff
    
    int32_t eax_24
    eax_24.b = edi[1]
    edi[1] = eax_24.b + (*(((ebx_48 - eax_24) << 2) + var_118)).b
    int32_t ebx_57
    ebx_57.b = *var_18
    int32_t eax_27 = edx_5 + *(((ebx_57 - edx_5) << 2) + eax_9)
    int32_t eax_28
    eax_28.b = not.b(eax_27.b)
    int32_t eax_29 = eax_28 + *(((eax_27 - eax_28) << 2) + var_d0)
    int32_t var_80
    int32_t ebx_66 = eax_29 + var_80
    int32_t ebx_74
    
    if (eax_29 + var_80 s< 0)
        if (ebx_66 s< 0xffffff01)
            ebx_66 = 0xffffff01
        
        int32_t ebx_77
        ebx_77.b = *edi
        ebx_74 = *((ebx_77 << 2) + *((ebx_66 << 2) + data_4ced578))
    else
        if (ebx_66 s> 0xff)
            ebx_66 = 0xff
        
        int32_t ebx_69
        ebx_69.b = *edi
        ebx_74 = *(((ebx_69 - 0xff) << 2) + *((ebx_66 << 2) + data_4ced578)) + 0xff
    
    int32_t eax_32
    eax_32.b = *edi
    result = eax_32.b + (*(((ebx_74 - eax_32) << 2) + var_118)).b
    *edi = result
    esi_4 = esi_1 + 4
    i = i_1
    i_1 -= 1
while (i != 1)
return result
